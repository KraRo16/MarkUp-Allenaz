...
export const AxisSpeed = () => {
  ...
  const handleSync = () => {
    ...
    if (!isLocked) { // если кнопка только что была нажата, устанавливаем значения всех слайдеров равными первому
  ...
  return (
    <div className={style.axis_speed_section}>
      ...
      <div className={style.axis_speed_blur}>
        ...
        <button
            className={style.axis_slider_locker_btn}
            onClick={handleSync}
          >
            {isLocked ? <BsFillLockFill size={26} color='red'/> : <BsFillUnlockFill size={26} color='green'/>}{/* изменяем текст на кнопке в зависимости от состояния */}
          </button>
